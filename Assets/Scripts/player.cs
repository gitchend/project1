...
public class player : charactor
{
    ...
    private bool is_effect_created = false;

    private int timer = 0;

    private int jump_charging = 0;
    private int jump_charging_max = 4;
    ...
    private bool movement_control ()
    {
        ...
        {
            if(!is_effect_created)
            {
                ec.create_effect (3, false, new Vector2(transform.position.x, transform.position.y - 0.4f), 0);
                is_effect_created = true;
            }
            if (get_anime_normalized_time () < 0.5)
            {

                ec.create_effect (4, false, new Vector2(transform.position.x + Random.value * 0.1f - 0.05f,  transform.position.y - 0.4f), 0);
            }
    ...
    private bool player_control ()
    {
        ...
        //roll
        if (roll_lock && (keydown_shift || keyboard_cache == KeyCode.LeftShift))
        {
            set_anime_para (3);
            if (moving_control_valid && direction != keydown_d)
            {
        ...
        if (attack_heavy_lock && (keydown_k || keyboard_cache == KeyCode.K) && mp_cost ())
        {
            set_anime_para (9);
            if (turn_lock && moving_control_valid && direction != keydown_d)
            {
        ...
        //use_item
        if (item_lock && (keydown_i || keyboard_cache == KeyCode.I) && item_map[item_code_list[item_index_now]] > 0)
        {
            if (is_climb_jump)
            {
